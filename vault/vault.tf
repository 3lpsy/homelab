    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          liveness_probe {
            # Liveness must treat a *sealed* vault as alive: the auto-unseal sidecar
            # does the unsealing, and restarting a sealed vault only re-seals it (the
            # cold-boot kill loop after a power outage). sealedcode/uninitcode=200 keep
            # liveness green as long as vault's HTTP listener responds at all; readiness
            # below stays strict (503 on sealed) so 3/3 still means truly unsealed.
            http_get {
