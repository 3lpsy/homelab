...
#        writes back rotated certs; openobserve-bootstrap in openobserve
...
resource "kubernetes_network_policy" "vault_cross_ns" {
  ...
  spec {
    ...
    # tls-rotator (cert rotation writes) + openobserve (bootstrap Job
    # writes service-account creds) + oidc (zitadel pat-sync sidecar
    # writes bootstrap PATs) → 8201. Kept in a single ingress block with
    # multiple `from` selectors — per memory feedback_netpol_same_port_pattern,
    # kube-router collapses same-port rules from different namespace_selectors
    # into one iptables rule with a unioned src ipset and only populates one
    # ns's pods if you split them. One block here forces the union explicitly.
    ingress {
      from {
        namespace_selector {
          match_labels = {
      ...
      from {
        namespace_selector {
          match_labels = {
            "kubernetes.io/metadata.name" = "openobserve"
          }
        }
      }
      from {
