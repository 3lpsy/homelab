...
#   artemis   -> host-artemis/    (kopia, provisioned by cluster — GPU agent
#                                  node; same backup_paths as delphi:
#                                  /etc, /root, /var/lib/rancher/k3s/storage
#                                  for any PVCs that migrate to artemis)
#
...
locals {
  backup_clients = {
    ...
    artemis   = "${var.backup_prefix_root}host-artemis/"
  }
}
