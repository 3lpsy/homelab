...
module "headscale-provision-dep" {
  ...
  ssh_user     = module.headscale-infra.ssh_user
  ssh_priv_key = trimspace(file(var.ssh_priv_key_path))
  admin_email  = "admin@${var.headscale_subdomain}.${var.headscale_magic_domain}"
}
...
module "headscale-provision-nginx" {
  ...
  # Host nginx writes JSON access logs to /var/log/nginx/access.log; the
  # EC2 OTel collector (data/otel/headscale-collector-config.yaml.tpl)
  # already tails that file and ships to OO. The oidc-public.conf vhost
  # included via /etc/nginx/conf.d inherits the http{}-level access_log.
  nginx_logging_block = templatefile("${path.root}/../data/nginx/_logging.conf.tpl", {
    log_level          = var.headscale_nginx_log_level
    access_log_enabled = var.headscale_nginx_access_log_enabled
    log_static_assets  = var.headscale_nginx_log_static_assets
    access_log_target  = "/var/log/nginx/access.log"
    error_log_target   = "/var/log/nginx/error.log"
  })
...
module "tailnet-infra" {
  ...
  personal_user_oidc_name = var.personal_user_oidc_name
  partner_user_oidc_name  = var.partner_user_oidc_name
  providers = {
    headscale = headscale
  }
