...
output "provisioner_preauth_key" {
  description = "Bootstrap preauth key for the human operator. Reusable, 30d expiry. Use during initial deployment before Zitadel exists; switch to OIDC on the personal device once services is applied."
  value       = headscale_pre_auth_key.provisioner.key
  sensitive   = true
}
