...
resource "null_resource" "install_deps" {
  ...
  provisioner "file" {
    content = templatefile("${path.root}/../data/server/50unattended-upgrades.tpl", {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo DEBIAN_FRONTEND=noninteractive apt install unattended-upgrades nginx curl unzip openssl age apache2-utils -y"
    ]
  }
  provisioner "remote-exec" {
    inline = [
