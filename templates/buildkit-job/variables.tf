...
variable "shared" {
  ...
  type = object({
    ...
    buildkit_debug = bool
    # Scheduling for every build Job — set once on the shared local so all
    # builds land together. Empty (the defaults) = scheduler's choice. Set to
    # pin builds to a node that carries a matching taint (e.g. artemis, the
    # gpu=true:NoSchedule compute node). `tolerations` is the raw manifest form.
