    ...
    spec = {
      ...
      template = {
        ...
        spec = {
          ...
          # Where build Jobs run — from the shared local. Empty = scheduler's
          # choice; set to pin builds to artemis (+ tolerate its gpu taint).
          nodeSelector = var.shared.node_selector
          tolerations  = var.shared.tolerations
