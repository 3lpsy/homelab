...
locals {
  ...
  # k8s Secret data keys must match [-._a-zA-Z0-9]+. Vault objectNames
  # may legitimately contain other chars (e.g. `password_<user>@<domain>`
  # for per-user creds keyed by Zitadel login_name); sanitize ONLY the
  # synced k8s data key. The Vault path + CSI file-mount filename
  # (driven by `objectName`) stay verbatim so consumers reading via
  # the file mount aren't affected.
  k8s_key_invalid_re = "/[^-._a-zA-Z0-9]/"

  config_so_list = local.has_config_secret ? [{
    secretName = local.config_secret_name
    type       = "Opaque"
    data = concat(
      [for k in local.config_keys : { objectName = k, key = replace(k, local.k8s_key_invalid_re, "_") }],
      [for e in var.extra_config_keys : { objectName = e.object_name, key = replace(e.object_name, local.k8s_key_invalid_re, "_") }],
    )
  }] : []
