...
# `litellm` (and a couple of its deps) add their own StreamHandler to the
# root logger at import time. Combined with `logging.basicConfig` that
# would add a second one, every record we emit propagates to root and
# gets printed by every attached handler — i.e. the duplicated `fetched`
# / `yt lookup` lines. Strategy: install exactly one handler on root,
# remove all others. Keep propagation on so pytest's caplog (which
# attaches its own handler to root) still captures records.
...
INGEST_CONCURRENCY = max(1, int(os.environ.get("INGEST_CONCURRENCY", "5")))

# DUPLICATE_MODE governs what happens when an incoming file's audio-stream
# md5 already exists in the library:
#   "skip"      — current behavior. Ack as success, drop the new file.
#   "overwrite" — run the new file through the LLM + tag pipeline, then
#                 delete the existing library entry and replace it with
#                 the freshly-tagged file. Useful when the original file
#                 has bad tags / wrong artist and a re-ingest is meant
#                 to fix it.
DUPLICATE_MODE = os.environ.get("DUPLICATE_MODE", "skip").lower()
if DUPLICATE_MODE not in {"skip", "overwrite"}:
    raise SystemExit(f"DUPLICATE_MODE must be 'skip' or 'overwrite', got {DUPLICATE_MODE!r}")
...
def write_tags(
    ...
    *,
    ...
    """Write artist (multi-value) + title + optional album/genre/year.

    Multi-value semantics by container:
      - Vorbis/Opus/FLAC: native multi-value tags. `audio["artist"] = [a, b]`
        produces multiple ARTIST comments; Navidrome reads them as collab.
      - ID3v2 (mp3, wav): TPE1 frame's `text` accepts a list. Navidrome
        v0.51+ + most Subsonic clients honor multi-value TPE1.
      - MP4 (m4a, aac): the `\\xa9ART` atom accepts a list of strings; mutagen
        encodes them as separate `data` entries in the same atom.
    """
    suffix = path.suffix.lower()
    artists_list = list(artists) if artists else []
    if not artists_list:
        # write_tags should not be called with empty artists — process_one
        # already quarantines those. Defensive: skip rather than crash.
        logger.warning("write_tags called with empty artists path=%s", path)
        return

    tags: dict[str, list[str]] = {"artist": artists_list, "title": [title]}
    if album:
        tags["album"] = [album]
    if genre:
        tags["genre"] = [genre]
    ...
    if suffix == ".mp3":
        ...
        # class wraps an ID3 instance. TPE1.text accepts a list — mutagen
        # encodes multi-value as null-separated within a single frame
        # (ID3v2.4 spec).
        audio = WAVE(path)
...
def load_schema() -> dict[str, Any]:
    ...
    return {
        "type": "object",
        "required": ["artists", "title", "confidence"],
        "properties": {
            "artists": {
                "type": "array",
                "items": {"type": "string"},
                "description": (
                    "Ordered list. Index 0 is the primary artist (used for the "
                    "library directory). Index 1+ are featured artists or "
                    "collaborators. Empty list when artist is unknown."
                ),
            },
            "title": {"type": "string"},
            "album": {"type": "string"},
...
class TagResult:
    ...
    def display_artist(self) -> str:
        """Single-string form for log lines and target_path quirks."""
        if not self.artists:
            return ""
        if len(self.artists) == 1:
            return self.artists[0]
        return f"{self.artists[0]} (feat. {', '.join(self.artists[1:])})"

    @classmethod
    def from_payload(cls, payload: dict[str, Any]) -> "TagResult":
        # Schema is `artists: [str]`. Older models / fallback content
        # sometimes still emit `artist: str` — accept both so we don't lose
        # responses to a one-character key change.
        raw_artists = payload.get("artists")
        if raw_artists is None and "artist" in payload:
            raw_artists = payload.get("artist")
        if isinstance(raw_artists, str):
            artists = _split_combined_artist(raw_artists)
        elif isinstance(raw_artists, list):
            # The model sometimes still emits ONE string with separators
            # (e.g. ["ANIZYZ x Mqx"], ["ANDONIS, Tevvez, Sung Jinwoo"]).
            # Split each element defensively so multi-value tagging works
            # regardless of how the model formats the response.
            artists = []
            for a in raw_artists:
                artists.extend(_split_combined_artist(str(a)))
        else:
            artists = []
        return cls(
            artists=artists,
...
# Common collab/feat separators we've seen the LLM produce as a single
# string. Order matters: longer/feat-style tokens first so we don't
# accidentally split on " x " inside "Tren Twins x …" before noticing
# a leading "feat." marker. ` x ` is the trickiest — uppercase X is
# treated separately because lowercase x can appear inside artist names
# (e.g. `1998Phantom`'s `x` is part of the name only when surrounded by
# letters, not whitespace; the regex below requires whitespace borders).
_ARTIST_SPLIT_RE = re.compile(
    r"\s+(?:feat\.|featuring|ft\.|&)\s+|\s+[xX]\s+|\s*,\s*",
    flags=re.IGNORECASE,
)


def _split_combined_artist(s: str) -> list[str]:
    """Split a single legacy-format artist string into a list.

    Handles the common LLM-emitted joins:
      "A x B"          -> ["A", "B"]
      "A & B"          -> ["A", "B"]
      "A feat. B"      -> ["A", "B"]
      "A ft. B"        -> ["A", "B"]
      "A featuring B"  -> ["A", "B"]
      "A, B, C"        -> ["A", "B", "C"]
      "A (feat. B)"    -> ["A", "B"]   (parens stripped first)

    Already-clean single names pass through untouched.
    """
    s = s.strip()
    if not s:
        return []
    # Strip the common parenthetical "(feat. X)" / "(ft. X)" wrapping
    # before splitting so we don't end up with a stray ")" in the tail.
    s = re.sub(
...
def _try_completion(
    *,
    ...
    if use_tool_call and tool_calls:
        ...
        # Log the raw tool-call args so quarantines can be debugged
        # without re-running the prompt by hand. Truncate aggressively —
        # tag JSON is short.
        raw_preview = (raw_args if isinstance(raw_args, str) else json.dumps(raw_args, ensure_ascii=False))
        logger.info("llm raw model=%s args=%s", routed_model, raw_preview[:600].replace("\n", " "))
...
def call_llm(
    ...
    duration_s: int | None,
    codec: str | None,
    youtube_hits: list[dict[str, Any]] | None = None,
) -> TagResult:
    ...
    yt_block = (
        json.dumps(youtube_hits, ensure_ascii=False, indent=2)
        if youtube_hits
        else "(no matches)"
    )
    ...
    system_msg = (
        ...
        "them null when unknown. The artists field is an ORDERED LIST: index 0 "
        "is the primary artist, index 1+ are featured/collab artists. "
        "Confidence must reflect how clearly artist+title can be read from the "
        "filename. Respond with the exact JSON shape expected by the schema; "
        "do not add prose."
    )
    messages = [
    ...
    return TagResult(artists=[], title="", album=None, genre=None, year=None, confidence=0.0)
...
async def process_one(client: httpx.AsyncClient, entry: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if audio_hash is not None:
            existing_relpath = await INDEX.contains(audio_hash)
            if existing_relpath is not None and DUPLICATE_MODE == "skip":
                logger.info(
                    "dedup hit file=%s hash=%s existing=%s — acking without ingest (mode=skip)",
                    filename, audio_hash, existing_relpath,
                )
                ...
                return {
                    ...
                    "status": "deduped",
                    "hash": audio_hash,
                    "existing": existing_relpath,
                }

        tags = read_existing_tags(tmp_path)
        probe = probe_audio(tmp_path)

        # YouTube enrichment: best-effort. Never raises — failures return
        # an empty list and the LLM tags from the filename alone.
        yt_query = Path(filename).stem
        yt_hits = await youtube_lookup.lookup_youtube(yt_query, top_k=3)
        ...
        # Always log what the model decided, success OR quarantine. Lets
        # operators inspect the LLM's actual output for files that end up
        # quarantined without re-running the prompt by hand.
        logger.info(
            "llm result file=%s artists=%r title=%r album=%r genre=%r year=%r confidence=%.2f",
            filename,
            tag_result.artists, tag_result.title, tag_result.album,
            tag_result.genre, tag_result.year, tag_result.confidence,
        )

        # Honest quarantine reason — separate the three failure modes so
        # operators can tell at a glance what the LLM actually missed.
        reasons: list[str] = []
        if tag_result.confidence < CONFIDENCE_THRESHOLD:
            reasons.append(f"confidence={tag_result.confidence:.2f}<{CONFIDENCE_THRESHOLD:.2f}")
        if not tag_result.artists:
            reasons.append("empty_artists")
        if not tag_result.title:
        ...
        if reasons:
            ...
            return {"file": filename, "status": "quarantined", "confidence": tag_result.confidence, "reason": reason}

        try:
            write_tags(
                tmp_path,
                artists=tag_result.artists,
        ...
        # Overwrite mode is "in-place": when the audio-stream md5 already
        # exists in the library, write the new file at the EXISTING path,
        # not at a path computed from the new tags. Navidrome IDs songs
        # by file path, so keeping the path stable preserves song IDs ->
        # playlist memberships, play counts, ratings, starred state.
        # Trade-off: the on-disk directory layout no longer reflects the
        # new artist/title, but Navidrome reads tags (not paths) for
        # display, so users see the corrected metadata in the UI.
        if existing_relpath is not None and DUPLICATE_MODE == "overwrite":
            final = MUSIC_PATH / existing_relpath
            final.parent.mkdir(parents=True, exist_ok=True)
            logger.info("overwrite-in-place file=%s path=%s", filename, existing_relpath)
        else:
            final = unique_path(target_path(tag_result.primary_artist, tag_result.title, suffix))
            final.parent.mkdir(parents=True, exist_ok=True)

        # tmp_path is on /tmp (different filesystem), so use shutil.move
        # which falls back to copy+unlink across filesystems. shutil.move
        # overwrites the destination if it exists (overwrite-in-place case).
        shutil.move(str(tmp_path), str(final))
        logger.info(
            "ingested file=%s -> %s artists=%s title=%s album=%s genre=%s year=%s confidence=%.2f%s",
            filename, final.relative_to(MUSIC_PATH),
            tag_result.artists, tag_result.title, tag_result.album,
            tag_result.genre, tag_result.year, tag_result.confidence,
            " (overwrote-in-place)" if existing_relpath is not None and DUPLICATE_MODE == "overwrite" else "",
        )
        if audio_hash is not None:
        ...
        ack = await client.post(f"/internal/dropzone/ack/{file_id}", timeout=POLL_TIMEOUT)
        ack.raise_for_status()
        status = "overwritten" if (existing_relpath is not None and DUPLICATE_MODE == "overwrite") else "ingested"
        return {
            "file": filename,
            "status": status,
            "artists": tag_result.artists,
...
async def poll_once(client: httpx.AsyncClient) -> None:
    ...
    logger.info("poll listed=%d concurrency=%d", len(entries), INGEST_CONCURRENCY)

    sem = asyncio.Semaphore(INGEST_CONCURRENCY)

    async def _gated(entry: dict[str, Any]) -> dict[str, Any] | None:
        async with sem:
            try:
                return await process_one(client, entry)
            except Exception:  # noqa: BLE001
                STATS.total_errors += 1
                logger.exception("process crash file=%s", entry.get("filename"))
                return None

    results = await asyncio.gather(*(_gated(e) for e in entries))
...
async def health(request: web.Request) -> web.Response:
    return web.json_response({
        ...
        "total_overwritten": STATS.total_overwritten,
        "total_errors": STATS.total_errors,
        "index_size": INDEX.last_rebuild_size,
        "index_last_rebuild_at": INDEX.last_rebuild_at,
        "duplicate_mode": DUPLICATE_MODE,
        "youtube_lookup_enabled": youtube_lookup.YOUTUBE_LOOKUP_ENABLED,
