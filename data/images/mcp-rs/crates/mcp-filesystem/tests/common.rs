// Same shape as the mcp-time/mcp-memory test helpers — duplicated because
// `CARGO_BIN_EXE_*` is set only inside the bin-owning crate's tests.
...
pub async fn initialize(srv: &Server) -> Result<(reqwest::Client, String), String> {
    initialize_with(srv.port, &srv.api_key).await
}

/// Initialize a session against a server using an arbitrary bearer — supports
/// multi-tenant tests that send different bearers against the same server.
...
pub async fn call_tool(
    ...
    call_tool_with(srv.port, &srv.api_key, client, session_id, name, args).await
}
