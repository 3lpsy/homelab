...
// Single SearXNG query -> raw result rows. Shared by the /pro POST /search
// route below and the getExaClient() shim, so both speak to the same endpoint
// with identical params. Throws on transport/HTTP error; callers decide how to
// surface it.
const searxngRaw = async (query: string, limit: number): Promise<Array<Record<string, unknown>>> => {
  ...
  return ((data.results ?? []) as Array<Record<string, unknown>>).slice(0, limit)
}

/**
 * SearXNG-backed stand-in for upstream's memoized Exa client.
 *
 * Upstream's authed `GET /search` route (backend/src/api/search.ts — NOT
 * overridden here) does `const client = deps.exaClient ?? getExaClient()` then
 * `client.search(q, { numResults, useAutoprompt, type })` and reads
 * `response.results[].{url,title,favicon,image}`. We must export `getExaClient`
 * with that shape or the module fails to load — that missing export is the
 * "Export named 'getExaClient' not found" boot crash when upstream main adds a
 * new consumer of pro/exa.ts that our overlay didn't yet mirror.
 *
 * Returns a lazy singleton whose `.search` hits SearXNG. Never null — SearXNG is
 * always configured in this deployment — so the route never 503s.
 * `useAutoprompt`/`type` are Exa-isms with no SearXNG analogue; ignored.
 */
type ExaLikeSearchResult = { url: string; title: string | null; favicon: string | null; image: string | null }
type ExaLikeClient = {
  search: (query: string, opts?: { numResults?: number }) => Promise<{ results: ExaLikeSearchResult[] }>
}

let exaClientSingleton: ExaLikeClient | null = null

export const getExaClient = (): ExaLikeClient => {
  if (!exaClientSingleton) {
    exaClientSingleton = {
      search: async (query, opts) => {
        const limit = Math.min(Math.max(opts?.numResults ?? 10, 1), 25)
        const rows = await searxngRaw(query, limit)
        return {
          results: rows.map((it) => ({
            url: String(it.url ?? ''),
...
export const exaPlugin = new Elysia({ name: 'exa' })
  ...
  .post(
    ...
    async ({ body }): Promise<SearchResponse> => {
      ...
      try {
        rows = await searxngRaw(body.query, body.max_results ?? 10)
      } catch (e) {
