...
| **C-0054** Cluster internal networking | `kube-system` namespace | Medium | Same rationale as C-0030/C-0260 above. The other three system namespaces (`default`, `kube-public`, `kube-node-lease`) get a deny-all NetworkPolicy via `services/system-namespaces-netpol.tf`. |
...
|---------|----------|----------|-----------|
| **CVE-2026-20884 / 24450 / 24660** | `libraw23t64` in `nextcloud:latest` (debian 13.4 base) | Critical | 18 trivy rows. No fixed version available in debian 13.4 upstream as of 2026-05-06. Library is pulled in by Nextcloud's image preview pipeline. Will resolve when `nextcloud:latest` rebases against a point release that backports. Re-evaluate quarterly. |
| **CVE-2026-40393** | `mesa-*` GPU userspace in `ghcr.io/blakeblackshear/frigate:stable-rocm` (debian 12 base) | Critical | 38 trivy rows. Unfixed in debian 12 upstream. Mesa is required for ROCm GPU inference on the Rembrandt 680M iGPU. Will resolve when Frigate rebases to debian 13. |
| **CVE-2026-40962** | `ffmpeg` in Frigate + Jellyfin upstream images | Critical | 46 trivy rows. Upstream image refresh cadence; not actionable from here. Both projects ship their own ffmpeg builds; we don't run our own ffmpeg layers. |
| **CVE-2025-7458** | `libsqlite3-0` in `frigate`, `immich-machine-learning`, `immich-postgres`, `pgvector` (all debian 12 base) | Critical | 5 trivy rows. Unfixed in debian 12 upstream. Will clear when each upstream image rebases to debian 13. |
