resource "kubernetes_config_map" "mcp_searxng_build_context" {
  metadata {
    name      = "mcp-searxng-build-context"
...
resource "kubernetes_manifest" "mcp_searxng_build" {
  manifest = {
    apiVersion = "batch/v1"
    kind       = "Job"
    metadata = {
      name      = local.mcp_searxng_build_job_name
      namespace = kubernetes_namespace.builder.metadata[0].name
    }
    spec = {
      backoffLimit = 2
          ...
          initContainers = [
            {
              ...
              env = [
                ...
                { name = "TS_KUBE_SECRET", value = "mcp-searxng-builder-tailscale-state" },
                ...
                { name = "TS_HOSTNAME", value = "mcp-searxng-builder" },
          ...
          containers = [
            {
              ...
              args = [
                ...
                "--output=type=image,name=${local.thunderbolt_registry}/mcp-searxng:latest,push=true",
              ]
              env = [
          ...
          volumes = [
            {
              name = "context"
              configMap = {
                name = kubernetes_config_map.mcp_searxng_build_context.metadata[0].name
              }
            },
            {
              name = "docker-config"
  ...
  depends_on = [
    ...
    kubernetes_config_map.mcp_searxng_build_context,
  ]
}
