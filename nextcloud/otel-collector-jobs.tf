# BuildKit Job that produces an OpenTelemetry Collector (contrib) image with
# `journalctl` available. Consumed by the OTel DaemonSet in `monitoring/`.
# Lives here because the `builder` namespace + registry pull-secret + tailscale
# auth for the builder all live in this deployment (see mcp-k8s-jobs.tf for
# the canonical pattern).

resource "kubernetes_config_map" "otel_collector_build_context" {
  metadata {
    name      = "otel-collector-build-context"
...
locals {
  otel_collector_dockerfile_hash = substr(sha256(
    file("${path.module}/../data/images/otel-collector/Dockerfile")
  ), 0, 8)
  otel_collector_build_job_name = "otel-collector-build-${local.otel_collector_dockerfile_hash}"
  otel_collector_image          = "${local.thunderbolt_registry}/otel-collector:latest"
}

resource "kubernetes_manifest" "otel_collector_build" {
  manifest = {
    apiVersion = "batch/v1"
    kind       = "Job"
    metadata = {
      name      = local.otel_collector_build_job_name
      namespace = kubernetes_namespace.builder.metadata[0].name
    }
    spec = {
      backoffLimit = 2
          ...
          initContainers = [
            {
              ...
              env = [
                ...
                { name = "TS_KUBE_SECRET", value = "otel-collector-builder-tailscale-state" },
                ...
                { name = "TS_HOSTNAME", value = "otel-collector-builder" },
          ...
          containers = [
            {
              ...
              args = [
                ...
                "--output=type=image,name=${local.otel_collector_image},push=true",
              ]
              env = [
          ...
          volumes = [
            {
              name = "context"
              configMap = {
                name = kubernetes_config_map.otel_collector_build_context.metadata[0].name
              }
            },
            {
              name = "docker-config"
  ...
  depends_on = [
    ...
    kubernetes_config_map.otel_collector_build_context,
  ]
}
