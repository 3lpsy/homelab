resource "kubernetes_config_map" "mcp_prometheus_build_context" {
  metadata {
    name      = "mcp-prometheus-build-context"
...
locals {
  mcp_prometheus_dockerfile_hash = substr(sha256(
    "${file("${path.module}/../data/images/mcp-prometheus/Dockerfile")}${file("${path.module}/../data/images/mcp-prometheus/server.py")}"
  ), 0, 8)
  mcp_prometheus_build_job_name = "mcp-prometheus-build-${local.mcp_prometheus_dockerfile_hash}"
}

resource "kubernetes_manifest" "mcp_prometheus_build" {
  manifest = {
    apiVersion = "batch/v1"
    kind       = "Job"
    metadata = {
      name      = local.mcp_prometheus_build_job_name
      namespace = kubernetes_namespace.builder.metadata[0].name
    }
    spec = {
      backoffLimit = 2
          ...
          initContainers = [
            {
              ...
              env = [
                ...
                { name = "TS_KUBE_SECRET", value = "mcp-prometheus-builder-tailscale-state" },
                ...
                { name = "TS_HOSTNAME", value = "mcp-prometheus-builder" },
          ...
          containers = [
            {
              ...
              args = [
                ...
                "--output=type=image,name=${local.thunderbolt_registry}/mcp-prometheus:latest,push=true",
              ]
              env = [
          ...
          volumes = [
            {
              name = "context"
              configMap = {
                name = kubernetes_config_map.mcp_prometheus_build_context.metadata[0].name
              }
            },
            {
              name = "docker-config"
  ...
  depends_on = [
    ...
    kubernetes_config_map.mcp_prometheus_build_context,
  ]
}
