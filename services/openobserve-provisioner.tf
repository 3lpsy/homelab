...
locals {
  ...
  oo_ntfy_internal_url = "http://ntfy.${kubernetes_namespace.ntfy.metadata[0].name}.svc.cluster.local:8080/${var.ntfy_alert_topic}"

  # NTFY_BASIC_B64 is a literal placeholder rendered into the ConfigMap;
  # provisioner.py expands it at runtime from NTFY_USER + NTFY_PASSWORD env
  # vars (sourced from Vault via CSI). Keeps the credential out of the
  # ConfigMap, so Velero backups never see it.
...
resource "vault_kubernetes_auth_backend_role" "openobserve_provisioner" {
  ...
  bound_service_account_namespaces = ["openobserve"]
      ...
      parameters = {
        ...
        objects = yamlencode([
          ...
          },
          {
            ...
            secretKey  = "password_openobserve"
          },
        ])
      }
    }
...
resource "kubernetes_manifest" "openobserve_provisioner_job" {
  manifest = {
    ...
    spec = {
      ...
      # would recreate it on the next apply. Job name is content-keyed by
      # sha256 of the dashboards/alerts/templates/destinations payload, so
      # leaving completed Jobs around is fine — re-applies are no-ops; cleanup
      # happens automatically on artifact change.
      template = {
        metadata = {
          labels = {
          ...
          containers = [
            {
              ...
              env = [
                ...
                { name = "NTFY_USER", value = "openobserve" },
                {
                  name = "OO_AUTH"
                  valueFrom = {
                    secretKeyRef = {
