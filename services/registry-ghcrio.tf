...
resource "kubernetes_config_map" "registry_ghcrio_nginx_config" {
  metadata {
    name      = "registry-ghcrio-nginx-config"
    namespace = kubernetes_namespace.registry_proxy.metadata[0].name
  }
  data = {
    "nginx.conf" = templatefile("${path.module}/../data/nginx/registry-proxy.nginx.conf.tpl", {
      ...
      upstream_port       = "5000"
      nginx_logging_block = local.nginx_logging_blocks["registry-ghcrio"]
    })
  }
}

resource "kubernetes_deployment" "registry_ghcrio" {
  metadata {
    name      = "registry-ghcrio"
    namespace = kubernetes_namespace.registry_proxy.metadata[0].name
  }
  ...
  spec {
    ...
    selector {
      match_labels = { app = "registry-ghcrio" }
    }

    template {
      metadata {
        labels = { app = "registry-ghcrio" }
        annotations = {
      ...
      spec {
        ...
        container {
          name  = "registry-ghcrio-tailscale"
          ...
          env {
            name  = "TS_HOSTNAME"
            value = var.registry_ghcrio_domain
          }
          env {
            name  = "TS_EXTRA_ARGS"
        ...
        volume {
          name = "registry-config"
          config_map {
            name = kubernetes_config_map.registry_ghcrio_config.metadata[0].name
          }
        }
        volume {
          name = "nginx-config"
          config_map {
            name = kubernetes_config_map.registry_ghcrio_nginx_config.metadata[0].name
          }
        }
        volume {
          name = "registry-ghcrio-tls"
...
resource "kubernetes_service" "registry_ghcrio" {
  metadata {
    name      = "registry-ghcrio"
    namespace = kubernetes_namespace.registry_proxy.metadata[0].name
  }
  spec {
    selector = { app = "registry-ghcrio" }
    port {
      name        = "https"
      port        = 443
      target_port = 443
