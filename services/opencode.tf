...
locals {
  ...
  # In-cluster pull-through mirror FQDNs (services/registry-{dockerio,ghcrio}.tf).
  # podman inside the pod routes docker.io/ghcr.io base-image pulls through
  # these to dodge Docker Hub's per-IP anonymous rate limit — the same proxies
  # the host containerd uses, but podman has its own registries.conf (it does
  # NOT read /etc/rancher/k3s/registries.yaml). Pinned to the Service ClusterIP
  # via host_aliases below; the FQDN form is kept so TLS SNI matches the
  # registry nginx cert SAN (same reasoning as builder-buildkitd-config.tf).
  opencode_dockerio_mirror_fqdn = "${var.registry_dockerio_domain}.${local.magic_fqdn_suffix}"
  opencode_ghcrio_mirror_fqdn   = "${var.registry_ghcrio_domain}.${local.magic_fqdn_suffix}"
...
# registries.conf drop-in for the in-pod podman. Mounted read-only at
# /etc/containers/registries.conf.d/01-homelab-mirrors.conf (the drop-in dir,
# leaving the image's package-default registries.conf intact). Routes
# docker.io / ghcr.io pulls through the in-cluster mirrors. Rendered from the
# registry-domain vars so it tracks the same source of truth as
# builder-buildkitd-config.tf — bump a domain there, both move together.
resource "kubernetes_config_map" "opencode_podman_registries" {
  metadata {
    name      = "opencode-podman-registries"
    namespace = kubernetes_namespace.opencode.metadata[0].name
  }
  data = {
    "01-homelab-mirrors.conf" = <<-EOT
      ...
      location = "docker.io"
      [[registry.mirror]]
      location = "${local.opencode_dockerio_mirror_fqdn}"

      [[registry]]
      prefix = "ghcr.io"
      location = "ghcr.io"
      [[registry.mirror]]
      location = "${local.opencode_ghcrio_mirror_fqdn}"
    EOT
  }
}
    ...
    template {
      metadata {
        ...
        annotations = {
          ...
          "podman-registries-hash"              = sha1(kubernetes_config_map.opencode_podman_registries.data["01-homelab-mirrors.conf"])
          "pkg-proxy-config-hash"               = sha1(jsonencode(kubernetes_config_map.opencode_pkg_proxy_config.data))
          "secret.reloader.stakater.com/reload" = "${module.opencode_tls_vault.config_secret_name},${module.opencode_tls_vault.tls_secret_name}"
        }
      }
      ...
      spec {
        ...
        # In-cluster pull-through mirrors for the in-pod podman (docker.io /
        # ghcr.io base images). FQDN→ClusterIP so podman's TLS SNI matches the
        # registry nginx cert SAN. Egress to the registry-proxy ns is opened
        # by opencode_to_registry_proxy in opencode-network.tf.
        host_aliases {
          ip        = kubernetes_service.registry_dockerio.spec[0].cluster_ip
          hostnames = [local.opencode_dockerio_mirror_fqdn]
        }
        host_aliases {
          ip        = kubernetes_service.registry_ghcrio.spec[0].cluster_ip
          hostnames = [local.opencode_ghcrio_mirror_fqdn]
        }
        host_aliases {
        ...
        # the entrypoint then `chown -R`s the writable mounts (PVC + podman
        # emptyDirs + .ssh) to uid 1001 — that runtime chown, not fsGroup, is
        ...
        container {
          ...
          # Rootless-podman storage. graphroot ($HOME/.local/share/containers)
          # is a dedicated DISK emptyDir, NOT the opencode-data PVC: the PVC is
          # local-path (node xfs) bind-mounted into an already-overlay rootfs,
          # and native overlay refuses an upperdir stacked on another overlay.
          # The emptyDir is a plain node-xfs dir = a clean overlay upperdir.
          # One-off images don't need persistence (re-pull via mirror is fast).
          volume_mount {
            name       = "podman-graphroot"
            mount_path = "/home/user/.local/share/containers"
          }
          # runroot — transient state (locks, mounts); tmpfs.
          volume_mount {
            name       = "podman-runroot"
            mount_path = "/run/containers"
          }
          # registries.conf drop-in (in-cluster docker.io/ghcr.io mirrors).
          volume_mount {
            name       = "podman-registries"
          ...
          # Rootless podman, UNPRIVILEGED. The container starts as root and
          # the entrypoint drops opencode to uid 1001 (`user`); rootless
          # podman then runs as `user`. The k3s nodes apply no seccomp profile,
          # so unshare(CLONE_NEWUSER) is allowed without privileged or extra
          # caps. allow_privilege_escalation MUST stay true: newuidmap/newgidmap
          # are setuid-root helpers that map the subuid range; setting it false
          # flips no_new_privs and the setuid bit is neutered (podman fails with
          # "cannot set up namespace"). The "no privesc to root" guarantee is
          # NOT no_new_privs — it's the image having no sudo and no setuid-root
          # shell binary (Dockerfile §5h strips all but newuidmap/newgidmap).
          # Add NO capabilities. Do NOT set run_as_user / run_as_non_root here
          # (the container must start as root for the entrypoint + sshd).
          # seccomp Unconfined is explicit (= the k3s default) to document the
          # userns dependency and survive a future cluster-wide SeccompDefault flip.
          security_context {
            privileged                 = false
        ...
        # Rootless-podman image store (overlay graphroot). Disk-backed
        # emptyDir, ephemeral by design — see the volume_mount comment.
        volume {
          name = "podman-graphroot"
          empty_dir {
            size_limit = "20Gi"
        ...
        # Rootless-podman runroot (locks/mountpoints). Memory-backed tmpfs.
        volume {
          name = "podman-runroot"
          empty_dir {
            medium = "Memory"
        ...
        volume {
          name = "podman-registries"
          config_map {
            name = kubernetes_config_map.opencode_podman_registries.metadata[0].name
          }
        }
        volume {
          name = "pkg-proxy-config"
