...
# Pull secret for the in-cluster registry. The docker.io / ghcr.io mirrors use
...
path "${data.terraform_remote_state.vault_conf.outputs.kv_mount_path}/data/npm/*" {
  capabilities = ["read"]
}
path "${data.terraform_remote_state.vault_conf.outputs.kv_mount_path}/data/crates/*" {
  capabilities = ["read"]
}
EOT
}
...
# Ingress for the in-pod podman in the opencode namespace pulling base images
# through these mirrors. Mirror egress lives in services/opencode-network.tf
# as opencode-to-registry-proxy.
resource "kubernetes_network_policy" "registry_proxy_from_opencode" {
  metadata {
    name      = "registry-proxy-from-opencode"
    namespace = kubernetes_namespace.registry_proxy.metadata[0].name
  }
