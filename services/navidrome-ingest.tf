    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          env {
            name  = "DUPLICATE_MODE"
            value = var.navidrome_ingest_duplicate_mode
          }
          env {
            name  = "YOUTUBE_LOOKUP_ENABLED"
            value = tostring(var.navidrome_ingest_youtube_lookup_enabled)
          }
          env {
            name  = "YOUTUBE_PROXY_URL"
            value = var.navidrome_ingest_youtube_proxy_url
          }
          env {
            name  = "INGEST_CONCURRENCY"
            value = tostring(var.navidrome_ingest_concurrency)
          }
          env {
            name  = "LOG_LEVEL"
            value = "info"
