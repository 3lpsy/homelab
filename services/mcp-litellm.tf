...
module "mcp_litellm_build" {
  ...
  build_args    = { BIN = "mcp-litellm", CRATES_REGISTRY = local.mcp_rs_crates_registry }
...
# Cross-ns egress: mcp-litellm → litellm:443. The mcp ns netpol-baseline
# default-denies cluster-internal egress, so without this rule the
# host_aliases-pinned hop to the litellm ClusterIP is blocked. Mirror
# ingress is `litellm_from_mcp_litellm` in services/litellm-network.tf.
resource "kubernetes_network_policy" "mcp_litellm_to_litellm" {
  metadata {
    name      = "mcp-litellm-to-litellm"
    namespace = kubernetes_namespace.mcp.metadata[0].name
  }
  spec {
