...
locals {
  buildkit_job_shared = {
    ...
    # Phase 4: run all in-cluster image builds on artemis (off delphi's
    # user-facing services). builder is stateless — registry-backed cache, no
    # PVC — so this is just scheduling. It doesn't use the GPU but must
    # tolerate artemis's gpu=true:NoSchedule taint to land there.
