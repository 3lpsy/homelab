module "mcp_prometheus_build" {
  ...
  build_args    = { BIN = "mcp-prometheus", CRATES_REGISTRY = local.mcp_rs_crates_registry }
...
# Cross-namespace egress for mcp-prometheus → Prometheus.
#
# mcp-prometheus is configured via `var.mcp_prometheus_url` to reach
# `http://prometheus.prometheus.svc.cluster.local:9090` directly (in-cluster
# DNS, no Tailscale hop). This allow is load-bearing today, not deferred.

resource "kubernetes_network_policy" "mcp_prometheus_to_prometheus" {
  metadata {
    name      = "mcp-prometheus-to-prometheus"
    namespace = kubernetes_namespace.mcp.metadata[0].name
  }
