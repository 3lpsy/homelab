# Provisioner: otelcol-contrib on the Headscale EC2 — install, configure,
# start. Ships journald + nginx logs to OpenObserve over the tailnet.
#
# Pure host-side provisioner via SSH (`null_resource`). No k8s objects
# created here, so nothing actually lands in the `provisioner` namespace
# from this file — the namespace exists as a parking spot for any future
# in-cluster provisioner Jobs that follow the same naming convention.
