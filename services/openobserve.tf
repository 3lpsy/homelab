...
locals {
  # Email shown on OO's login screen. Matches the Zitadel personal-user
  # form (`<user_name>@<magic_domain>`, e.g. `jim@magic`) for consistency
  # with other services. NOTE: changing this on an already-seeded
  # instance does NOT rename the existing root user — OO caches it on
  # first boot. Manual rename: `./openobserve reset -c root` inside the
  # pod after the new env value is in effect, OR wipe the data PVC.
  openobserve_root_email    = "${var.zitadel_personal_user.user_name}@${var.headscale_magic_domain}"
...
resource "kubernetes_persistent_volume_claim" "openobserve_data" {
  # No prevent_destroy: deployment annotation already excludes this PVC
  # from FSB and the comment confirms OO is happy starting on an empty
  # store. Losing this PVC means re-running the bootstrap + provisioner
  # Jobs (the doc'd recovery path) — see prior PVC wipe runbook.
  metadata {
...
resource "kubernetes_config_map" "openobserve_nginx" {
  ...
  data = {
    "nginx.conf" = templatefile("${path.module}/../data/nginx/openobserve.nginx.conf.tpl", {
      ...
      nginx_logging_block = local.nginx_logging_blocks["openobserve"]
    })
  }
}
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        # Wait for Vault CSI to materialize the root creds. CSI writes
        # the secret atomically, so a present root_password implies the
        # rest of the keys too.
        init_container {
